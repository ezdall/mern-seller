...
export default function StripeConnect(){
  ...
  useEffect(() => {
    ...
    if(code){
      setValues(prevValues => ({...prevValues, connecting: true, error: false}))
      // post call to stripe, get credentials and update user data

      stripeUpdate(code, signal)
        .then((data) => {
          if (data.isAxiosError) {
           setValues(prevValues => ({...prevValues, error: true, connected: false, connecting: false}))
          } else {
          setValues(prevValues => ({...prevValues, connected: true, connecting: false, error: false}))
        }
      })
    }
    return () => {
