...
export const update = async (params, product) => {
  try {
    const response = await axios.patch(`/api/order/status/${params.shopId}`, product, {
      headers: {
        authorization: `Bearer ${accessToken}`
      },
    })
...
export const cancelOrder = async (params, product) => {
  try {
    const response = await axios.patch(`/api/order/${params.shopId}/cancel/${params.productId}`, product, {     
      headers: {
        authorization: `Bearer ${accessToken}`
      }
    })
...
export const processCharge = async (params, product) => {
  try {
    const response = await axios.patch(
      `/api/order/${params.orderId}/charge/${params.userId}/${params.shopId}`, product,
      {
        headers: {
        authorization: `Bearer ${accessToken}`
      },
